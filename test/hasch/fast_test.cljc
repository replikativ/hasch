(ns hasch.fast-test
  (:require [hasch.fast :refer [edn-hash uuid uuid4 squuid b64-hash hash->str hash-ref
...
;; =============================================================================
;; HashRef - structural/merkle hashing
;; =============================================================================

(deftest hashref-determinism-test
  (testing "HashRef produces deterministic hashes"
    (let [ref1 (hash-ref {:a 1 :b 2})
          ref2 (hash-ref {:b 2 :a 1})]
      (is (= (hash->unsigned-seq (edn-hash [ref1]))
             (hash->unsigned-seq (edn-hash [ref2])))
          "HashRefs from equal values should produce equal parent hashes"))))

(deftest hashref-different-from-flat-test
  (testing "HashRef hash differs from flat hash"
    (let [subtree {:a 1 :b 2}
          ref (hash-ref subtree)]
      (is (not= (hash->unsigned-seq (edn-hash [subtree]))
                (hash->unsigned-seq (edn-hash [ref])))
          "HashRef in a tree should differ from the inlined value"))))

(deftest hashref-structural-sharing-test
  (testing "Changing one subtree with HashRef avoids rehashing unchanged subtrees"
    (let [sub-a {:name "Alice" :scores [1 2 3]}
          sub-b {:name "Bob" :scores [4 5]}
          ref-a (hash-ref sub-a)
          ref-b (hash-ref sub-b)
          root1 (hash->unsigned-seq (edn-hash {:users [ref-a ref-b]}))
          ;; Change sub-a, reuse ref-b
          sub-a' {:name "Alice" :scores [1 2 3 4]}
          ref-a' (hash-ref sub-a')
          root2 (hash->unsigned-seq (edn-hash {:users [ref-a' ref-b]}))]
      (is (not= root1 root2)
          "Root hash should change when a subtree changes")
      ;; Verify ref-b was reused (same bytes)
      (is (= (vec (.-hash-bytes ref-b))
             (vec (.-hash-bytes ref-b)))))))

(deftest hashref-cross-module-error-test
  (testing "hasch.benc/HashRef should not be usable in hasch.fast"
    (let [benc-ref (benc/->HashRef #?(:clj (byte-array [1 2 3])
                                      :cljs #js [1 2 3]))]
      ;; benc/HashRef implements PHashCoercion, not PStreamHash
      ;; It should cause an error or fall through to default when used in hasch.fast
      (is (thrown? #?(:clj Exception :cljs js/Error)
                   (edn-hash [benc-ref]))
          "hasch.benc/HashRef should not work in hasch.fast"))))
