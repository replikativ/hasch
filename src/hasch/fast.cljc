...
(def ^:const magic-hash-ref   (unchecked-byte 19))
...
#?(:clj
   (extend-protocol PStreamHash
     ...
     (-update-digest [this md]
       (cond
         ;; Guard: benc/HashRef is a record but must NOT be used in hasch.fast
         (instance? hasch.benc.HashRef this)
         (throw (ex-info "Cannot use hasch.benc/HashRef in hasch.fast. Use hasch.fast/HashRef instead."
                         {:type (type this)}))
...
;; =============================================================================
;; HashRef - pre-computed hash for structural/merkle hashing
;; =============================================================================

(defrecord HashRef [hash-bytes]
  #?@(:clj
      [PStreamHash
       (-update-digest [_this md]
                       (update-byte md magic-hash-ref)
                       (update-int md (alength ^bytes hash-bytes))
                       (update-bytes md hash-bytes))]
      :cljs
      []))
...
#?(:cljs
   (defn- cljs-hash-value
     ...
     (cond
       ...
       (map? val)
       (cond
         ;; Guard: benc/HashRef must NOT be used in hasch.fast
         (instance? benc/HashRef val)
         (throw (ex-info "Cannot use hasch.benc/HashRef in hasch.fast. Use hasch.fast/HashRef instead."
                         {:type (type val)}))
       ...
       ;; HashRef - pre-computed hash for structural/merkle hashing
       (instance? HashRef val)
       (let [hb (.-hash-bytes val)]
         (cljs-update-byte hasher magic-hash-ref)
         (cljs-update-int hasher (alength hb))
         (cljs-update-bytes hasher hb))
...
(defn hash-ref
  "Create a HashRef from a value. Computes edn-hash and wraps the result
   ...
   Note: HashRef values from hasch.fast can only be used in hasch.fast
   computations (not hasch.core), as the encoding formats differ."
  ([val] (->HashRef (edn-hash val)))
  ([val md-create-fn] (->HashRef (edn-hash val md-create-fn))))
